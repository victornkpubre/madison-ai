...
Just an email address and an application password.
...
def save_smtp_account(email: str,
    ...
    if settings.database_url:
        from database import upsert_smtp_account
        upsert_smtp_account(
            email,
            record["password"],
            record["display_name"],
...
def get_smtp_account(email: str) -> dict | None:
    ...
    if settings.database_url:
        from database import get_smtp_account as db_get
        return db_get(email)
...
def list_email_accounts() -> list[dict]:
    ...
    if settings.database_url:
        from database import list_email_accounts as db_list
        return db_list()
    return [
...
async def verify_smtp_credentials(email: str,
    ...
    def _test():
        try:
            ...
            return {"ok": False,
                    "error": "Authentication failed. Check the application password is correct."}
...
async def send_smtp(account_email: str,
    ...
    def _send():
        ...
        try:
            ...
            return {"ok": False,
                    "error": "Authentication failed — application password may have been revoked."}
